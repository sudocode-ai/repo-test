...
def test_direct_counter_usage(sample_code):
    ...
    counts = count_symbols_aliased(sample_code)
...
def test_counter_identical_results(sample_code):
    ...
    direct_results = count_symbols_aliased(sample_code)
    class_results = AliasedClassToTest().count_symbols(sample_code)
    
    assert direct_results == class_results

def test_empty_code():
    empty_code = ""
    counts = count_symbols_aliased(empty_code)
...
global_var2 = 5
"""
    counts = count_symbols_aliased(complex_code)
